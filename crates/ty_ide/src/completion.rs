...
impl UserQuery {
    ...
    fn match_quality(&self, completion: &CompletionBuilder<'_>) -> MatchQuality {
        if completion.is_type_check_only || completion.deprecated {
...
impl Relevance {
    ...
    fn new(_ctx: &CollectionContext, query: &UserQuery, c: &CompletionBuilder) -> Relevance {
        Relevance {
            ...
            match_quality: query.match_quality(c),
        }
    }
}
...
fn add_import_completions_impl<'db>(
    ...
    completions: &mut Completions<'db>,
    semantic_completions: impl IntoIterator<Item = SemanticCompletion<'db>>,
    mut module_dependency_kind: impl FnMut(&SemanticCompletion<'db>) -> Option<ModuleDependencyKind>,
) {
...
impl Ord for CompletionRanker<'_> {
    fn cmp(&self, rhs: &CompletionRanker<'_>) -> Ordering {
        // First, order by relevance, and break those ties by alphabetically by symbol name.
        (&self.0.relevance, &self.0.name)
            .cmp(&(&rhs.0.relevance, &rhs.0.name))
            // Second, prefer symbols from standard library modules over those from third-party.
            .then_with(|| {
                self.0
                    .module_dependency_kind
                    .cmp(&rhs.0.module_dependency_kind)
            })
            // Finally, break any remaining ties by alphabetically by module name.
