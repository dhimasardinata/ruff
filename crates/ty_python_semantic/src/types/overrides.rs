...
fn check_class_declaration<'db>(
    ...
    if !is_private_member {
        for class_base in class.iter_mro(db).skip(1) {
            ...
            if configuration.check_missing_overrides() && missing_override_target.is_none() {
                missing_override_target = MissingOverrideTarget::for_superclass(
                    db,
                    superclass,
                    superclass_scope,
                    superclass_symbol_id,
                    superclass_type,
                    is_synthesized_superclass_member,
                );
            }
...
impl<'db> MissingOverrideTarget<'db> {
    fn for_superclass(
        ...
        is_synthesized_superclass_member: bool,
    ) -> Option<Self> {
        // `@override` is required for user-authored concrete superclass behaviour, but not for
        // built-in `object`/`type` hooks, ty-generated fallback members, or interface-only targets.
        if is_synthesized_superclass_member
            || superclass.is_object(db)
            || superclass.is_known(db, KnownClass::Type)
            || Self::is_interface_override_target(db, superclass, superclass_type)
        {
            return None;
        }
    ...
    fn is_interface_override_target(
        db: &'db dyn Db,
        superclass: ClassType<'db>,
        superclass_type: Type<'db>,
    ) -> bool {
