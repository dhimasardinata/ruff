...
fn mdtest_rule_selection(rules: Option<&Rules>) -> RuleSelection {
    ...
    // rules.
    //
    // `missing-override-decorator` is an exception: we have chosen to keep it opt-in to minimize
    // unrelated test churn while we migrate even our own tests to use that rule by default.
